...
import { ArrowLeft, CheckCircle2, Clock3, Fingerprint as FingerprintIcon, LoaderCircle, ShieldCheck, WifiOff } from 'lucide-react';
  ...
  return <main className="relative min-h-screen overflow-hidden bg-slate-950 px-4 py-6 text-white sm:px-8">
    ...
    <div className="relative mx-auto flex min-h-[calc(100vh-3rem)] max-w-6xl flex-col">
      <header className="flex items-center justify-between gap-4">
        ...
        <a href="/overview?view=employees" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-indigo-100 hover:bg-white/10"><ArrowLeft className="h-4 w-4" /> Admin workspace</a>
      </header>

      <section className="grid flex-1 items-center gap-10 py-10 lg:grid-cols-[.9fr_1.1fr]">
        <div>
          <div className="flex items-center gap-3 text-indigo-200"><Clock3 className="h-5 w-5" /><span className="text-sm font-semibold uppercase tracking-[.18em]">{now.toLocaleDateString('en-PH', { timeZone: 'Asia/Manila', weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })}</span></div>
          ...
          <h1 className="mt-8 text-3xl font-bold sm:text-4xl">Time in or out with your registered finger.</h1>
          <p className="mt-4 max-w-xl leading-7 text-indigo-100/65">Place a registered finger on the reader. WorkPulse automatically identifies the employee and records time in or out.</p>
          <div className="mt-7 flex items-center gap-2 text-sm text-emerald-300"><ShieldCheck className="h-4 w-4" /> FingerJet server-side identity matching</div>
        </div>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/[.07] p-6 shadow-2xl backdrop-blur-xl sm:p-10">
          <div className="flex flex-col items-center text-center">
            ...
            {result ? <>
              <p className="mt-6 text-sm font-semibold uppercase tracking-[.18em] text-emerald-300">{result.action === 'time-in' ? 'Time in recorded' : 'Time out recorded'}</p>
              <h2 className="mt-2 text-3xl font-bold">{result.record.name}</h2>
              ...
              <p className="mt-6 text-sm font-semibold text-indigo-100/70">Remove your finger. The kiosk will arm itself for the next employee.</p>
            </> : <>
              <h2 className="mt-6 text-xl font-bold">{message}</h2>
              <p className="mt-2 text-sm text-indigo-100/60">No button is needed. Keep your finger flat and lift it after recognition.</p>
              {error && <p className="mt-4 rounded-xl border border-rose-300/20 bg-rose-400/10 px-4 py-3 text-sm text-rose-200">{error}</p>}
            </>}
          </div>
        </div>
      </section>
