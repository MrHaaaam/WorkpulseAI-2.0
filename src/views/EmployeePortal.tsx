...
type EmployeeProfile = {
  id: string; name: string; email?: string; phone?: string; address?: string; role: 'regular' | 'extra'; status: string; biometricStatus: string;
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_1fr]">
      ...
      <main className="min-w-0">
        ...
        {section === 'profile' && <Panel title="My Profile" subtitle="Contact an administrator to change protected information"><div className="grid gap-4 sm:grid-cols-2"><ProfileItem label="Employee ID" value={profile.id} /><ProfileItem label="Employment type" value={profile.role} /><ProfileItem label="Email" value={profile.email || 'Not provided'} /><ProfileItem label="Phone" value={profile.phone || 'Not provided'} /><ProfileItem label="Address" value={profile.address || 'Not provided'} /><ProfileItem label="Biometric status" value={profile.biometricStatus} /></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><LeaveBalance title="Casual leave" balance={profile.casualLeave} /><LeaveBalance title="Sick leave" balance={profile.sickLeave} /></div></Panel>}
      </main>
    </div>
  </div>
}
